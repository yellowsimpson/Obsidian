...
- 끝단의 x,y를 알기 위해 함수를 이용해서
- 계산이 간단하고 시각적 직관이 강한것
- 복잡한 구조에서는 활용이 어려움


### DH 파라미터

- 4개의 파라미터로 활용해서 특단의 포워드 매트릭스를 계산한다.
- 결국 끝단을 알아가는 것이 목적
## 3. 역기구학

## 역기구학 (Inverse Kinematics)

- 오차가 작아질 때까지 반복해서 오차가 0으로 갈때까지 계산을 하는 근사값을 찾아서 계산하는 방법
    - 와 이거 진짜 어려움 너무 수학이야
- 가장 많이 활용하는 방법
    - 수치적 방법에서 핵심은 Jacobian
