...
image_edges = cv2.Canny(gray_image, 100, 300)  # 100과 200은 낮은/높은 임계값
template_edges = cv2.Canny(template_image, 100, 300)

# 템플릿 이미지 크기
template_height, template_width = template.shape[:2]

# 템플릿 매칭
result = cv2.matchTemplate(image_edges, template_edges, cv2.TM_CCORR)
...
cv2.imshow('Template Image', template_edges)
